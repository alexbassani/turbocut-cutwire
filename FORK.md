...
| Visual do instalador | `resources/windows/wizard-large.bmp`, `wizard-small.bmp` (gerados; fonte em `branding/TurboCut-instalador-arte.png`) | Banner e logo do assistente do Inno Setup com a marca TurboCut, no lugar do visual padrão |
| Splash de abertura | `src/main.cpp` (classe `TurboSplash`), `resources/splash.png`, `CMakeLists.txt` | Tela preta com o wordmark em fade e barra prata indeterminada enquanto fontes/caches/QML carregam; fecha quando a janela principal aparece |
